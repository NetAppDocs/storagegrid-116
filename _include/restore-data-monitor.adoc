...
+
IMPORTANT: The `show-replicated-repair-status` option is available for technical preview in StorageGRID 11.6. This feature is under development, and the value returned might be incorrect or delayed. To determine if a repair is complete, use *Awaiting – All*, *Repairs Attempted (XRPA)*, and *Scan Period -- Estimated (XSCM)* as described in xref:monitor-repairs.adoc[Monitor repairs].
