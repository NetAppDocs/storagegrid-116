...
* If you will use the HTTPS or HTTP protocol and you want to use a proxy server, you must have xref:configuring-admin-proxy-settings.adoc[configured an Admin proxy server].
...
To determine when the weekly AutoSupport message will be sent, see *Next Scheduled Time* under *Weekly AutoSupport* on the *AutoSupport* > *Results* tab. 
...
By default, AutoSupport checks to determine if software updates are available for your system. You can optionally disable these checks.
