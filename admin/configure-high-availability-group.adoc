...
* If you select interfaces on different types of nodes, an informational note appears. You are reminded that if a failover occurs, services provided by the previously active node might not be available on the newly active node. For example, a backup Gateway Node would not be able to provide HA protection of Admin Node services. Similarly, a backup Admin Node would not be able to perform all of the maintenance procedures that the primary Admin Node could provide.
...
+
If the HA group includes more than one interface and the Primary interface fails, the VIP addresses move to the first Backup interface in the priority order. If that interface fails, the VIP addresses move to the next Backup interface, and so on.
...
If you will use this HA group for load balancing, create a load balancer endpoint to determine the port and network protocol and to attach any required certificates: xref:managing-load-balancing.adoc[Manage load balancing].
