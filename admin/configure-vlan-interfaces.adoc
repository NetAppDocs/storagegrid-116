...
* A parent interface can be the Grid Network (eth0), the Client Network (eth2), or an additional trunk interface for the VM or bare-metal host (for example, ens236).
...
* Edit the HA group. Move the VLAN interface you want to remove to the bottom of the priority list. Wait until communication is established on the new node, remove the interface from the HA group, and then delete the VLAN interface on that node.
