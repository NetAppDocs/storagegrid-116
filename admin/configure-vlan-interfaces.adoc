...
=== Requirements for parent interfaces
...
* If the VLAN interface is used in an HA group, you can't change its VLAN ID.
* If the VLAN interface is used in an HA group, you can only remove a parent interface if the parent interface is not used. 
+
