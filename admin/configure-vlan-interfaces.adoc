...
* Edit the HA group. Move the VLAN interface you want to remove to the bottom of the failover list. Wait until communication is established on the new node, remove the interface from the HA group, and then delete the VLAN interface on that node.
