...
See the information about xref:using-storagegrid-security-certificates.adoc[general security certificate use] and xref:configuring-custom-server-certificate-for-grid-manager-tenant-manager.adoc[configuring custom server certificates]. If you are using a key management server (KMS) to protect the data on specially configured appliance nodes, see the xref:kms-adding.adoc[information about uploading a KMS client certificate].
...
== Copy or download client certificates

You can copy or download a client certificate for use elsewhere.
...
NOTE: To remove multiple certificates, select each certificate to remove on the Client tab and then select *Actions* > *Remove*.
