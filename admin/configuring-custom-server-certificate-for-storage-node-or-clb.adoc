...
You can revert to using the default S3 and Swift API certificate for S3 and Swift client connections to Storage Nodes and to the deprecated Connection Load Balancer (CLB) service on Gateway Nodes. However, you cannot use the default S3 and Swift API certificate for a load balancer endpoint.
