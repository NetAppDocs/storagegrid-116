...
See xref:../admin/configuring-custom-server-certificate-for-storage-node-or-clb.adoc[Configure S3 and Swift API certificates].
+
...
|The S3 and Swift API certificate is generated during StorageGRID installation and is signed by the grid certificate authority ("`grid CA`"). If you want to select this global certificate for a load balancer endpoint, you must have already replaced the default certificate with a custom certificate signed by an external certificate authority.
...
You can change an endpoint's  name and binding mode. For a secured (HTTPS) endpoint, you can also view or change the security certificate. If you need to change the service type (S3 or Swift) or the protocol (HTTP or HTTPS), you must create a new endpoint.

.Steps

. Select *CONFIGURATION* > *Network* > *Load balancer endpoints*.
+
The table shows details for all existing load balancer endpoints. You can edit endpoint details using the *Actions* menu or from the details page.

[cols="1a, 2a,2a" options="header"]
|===
|Detail to edit | Actions menu | Details page
...
|.. Select the name of endpoint from the table.
.. Select the edit icon image:../media/icon_edit_tm.png[Edit icon] next to the name.
.. Select *Save*.
...
|.. Select the name of endpoint from the table.
.. Select the *Edit binding mode* button on the details page.
.. Update the binding mode as required.
.. Select *Save changes*.
...
|.. Select the name of endpoint from the table.
.. Select the *Certificate* tab on the details page.
...
. Select *CONFIGURATION* > *Network* > *Load balancer endpoints*.
+
The table shows details for all existing load balancer endpoints. You can remove one or more endpoints using the *Actions* menu or you can remove a single endpoint from the details page.
...
|.. Select the check box for each endpoint you want to remove. 
.. Select *Actions* > *Remove*.
.. Select *OK*. A green success banner appears on the Load balancer endpoints page.
