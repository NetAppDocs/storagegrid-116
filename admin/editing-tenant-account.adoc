...
. Select *Tenants*.
+
The Tenants page appears and lists all existing tenant accounts.
+
...
+
The *Edit the tenant* wizard appears. This example is for a grid that does not use single sign-on (SSO). This tenant account has not configured its own identity source.
+
