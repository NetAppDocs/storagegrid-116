---
permalink: admin/task_kms_editing.html
sidebar: sidebar
keywords: 
...
Key Name
a|
The exact key alias for the StorageGRID client in the KMS. Must be between 1 and 255 characters.     You only need to edit the key name in rare cases. For example, you must edit the key name if the alias is renamed in the KMS or if all versions of the previous key have been copied to the version history of the new alias.
+
CAUTION:
+
...
. If you are configuring a KMS cluster, select the plus sign image:../media/plus_sign_black_on_white.png[Icon Plus Sign] to add a hostname for each server in the cluster.
. Select *Next*.
+
