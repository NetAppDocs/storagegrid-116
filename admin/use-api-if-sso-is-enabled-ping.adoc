 ...
 ** The SSO username
 ** The domain where StorageGRID is installed. This field is not used for Ping Federate. You can leave it blank or enter any value.
...
+
This example shows a POST request for a signed authentication URL for `TENANTACCOUNTID`. The results will be passed to the Python -m json.tool to remove the JSON encoding.
+
[source,bash]
...
Set-Cookie: MSISSignoutProtocol=U2FtbA==; expires=Tue, 12 Oct 2021 22:35:03 GMT; path=/adfs; HttpOnly; Secure
----
