...
.. In the Enterprise Application section, specify an *Enterprise application name* for StorageGRID. This value controls the name you use for each enterprise application in Azure AD.
...
.. Follow the steps in xref:../admin/creating-enterprise-application-azure.adoc[Creating enterprise applications in Azure AD] to create an enterprise application for each Admin Node listed in the table.

.. Copy the federation metadata URL for each enterprise application from Azure and paste it into the corresponding *Federation metadata URL* field in StorageGRID.
...
. Go to xref:../admin/testing-relying-party-trusts.adoc[Test relying party trusts].
...
. Follow the steps in xref:../admin/creating-enterprise-application-azure.adoc[Creating enterprise applications in Azure AD] to upload the SAML metadata file for each Admin Node into its corresponding Azure enterprise application.
...
. xref:../admin/creating-sp-connection-ping.html.adoc[Create one or more service provider (SP) connections for StorageGRID]. Use the SP connection ID for each Admin Node (shown in the table on the StorageGRID Single Sign-on page) and the SAML metadata you downloaded for that Admin Node. 
+
...
+
. When you have confirmed SSO sign-in and logout operations for each relying party trust and you are ready to enforce the use of SSO for StorageGRID, go to xref:../admin/enabling-single-sign-on.adoc[Enable single sign-on].
...
Repeat these steps for each enterprise application you created.
...
+
. When you have confirmed SSO is successful for each enterprise application and you are ready to enforce the use of SSO for StorageGRID, go to xref:../admin/enabling-single-sign-on.adoc[Enable single sign-on].
...
. Select the next link to test the SP connections for any other Admin Nodes in your grid.

. When you have confirmed SSO is successful for each SP connection and you are ready to enforce the use of SSO for StorageGRID, go to xref:../admin/enabling-single-sign-on.adoc[Enable single sign-on].
