...
.. Follow the steps in xref:../admin/creating-enterprise-application-azure.adoc[Creating enterprise applications in Azure] to create an enterprise application for each Admin Node listed in the table.

.. Copy the federation metadata URL for the enterprise application in Azure and paste it into the corresponding *Federation metadata URL* field in StorageGRID.
...
.. Select the button to download the SAML metadata for the Admin Node you are currently signed in to. 

You must upload the SAML metadata for each Admin Node into the corresponding Azure enterprise application.
...
. Select and test one link at a time from the Sandbox mode notification.
+
