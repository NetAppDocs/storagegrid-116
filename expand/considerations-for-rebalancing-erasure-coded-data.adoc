...
* Does not evaluate replicated data usage when determining where to move erasure-coded data. That is, the procedure does not look at the relative amount of replicated data on each Storage Node and does not choose nodes with lower overall usage.
