...
By default, audit information is sent to the audit log on Admin Nodes. You can skip this procedure if either of the following applies:

* Audit logs are being sent to an external syslog server instead of to Admin Nodes.
* Audit messages are saved only on the local nodes that generated them. 
