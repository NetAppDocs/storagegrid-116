...
. Select network interfaces for one or more Admin Nodes or Gateway Nodes. You can select the Grid Network interface (eth0), Admin Network interface (eth1), Client Network interface (eth2), or a VLAN interface.
+
IMPORTANT: If you plan to use a VLAN, you must create the VLAN interface before you create the HA group. You can use a VLAN for FabricPool only if StorageGRID is using VLANs for client traffic and the FabricPool VLAN is available on the trunked interface. 
...
. Specify one interface to be the Primary interface and determine the failover order for any Backup interfaces. The Primary interface is the active interface unless a failure occurs.
