...
+
NOTE: In FabricPool configurations, the recommended bucket consistency level is "`read-after-new-write.`" The "`available`" consistency level is not recommended. See xref:../tenant/changing-consistency-level.adoc[Changing the consistency level].
