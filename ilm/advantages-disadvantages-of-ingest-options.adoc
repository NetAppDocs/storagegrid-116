...
NOTE: Before selecting a consistency level, read the full description of consistency controls in the instructions for xref:../s3/s3/consistency-controls.adoc[S3] or xref:../swift/storagegrid-swift-rest-api-operations.adoc[Swift] client applications. You should understand the benefits and limitations before changing the default value.
