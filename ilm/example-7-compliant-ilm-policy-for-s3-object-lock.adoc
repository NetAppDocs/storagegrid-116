...
. A compliant rule that uses erasure coding to protect objects larger than 1 MB in a specific bucket with S3 Object Lock enabled. The objects are stored on Storage Nodes from day 0 to forever.
...
* The first rule will only match test objects that are larger than 1 MB in the bucket bank-records for the Bank of ABC tenant.
