...
* An StorageGRID bucket that has S3 Object Lock enabled can have a default retention period, or the S3 client application can optionally specify a retention date and legal hold setting for each object version that is added to that bucket.
...
* The S3 client application can specify retention settings for each object that needs to be protected by S3 Object Lock. If no retention settings are specified for an object, and a default retention mode and default retention period are set at the bucket level, the default bucket-level retention settings are applied to each object version added to the bucket.
