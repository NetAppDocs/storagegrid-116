---
permalink: ilm/task_creating_a_storage_pool.html
sidebar: sidebar
keywords: 
...
* For erasure coding at three or more sites, create one storage pool that includes an entry for each site. For example, if you want to erasure code objects across three sites, create one storage pool. Select the plus icon image:../media/plus_sign_black_on_white.gif[insert icon] to add an entry for each site.
+
IMPORTANT: Do not include the default All Sites site in a storage pool that will be used in an Erasure Coding profile. Instead, add a separate entry to the storage pool for each site that will store erasure coded data. See step <<STEP_ADD_SP_ENTRIES,#STEP_ADD_SP_ENTRIES>> for an example.
...
xref:concept_guidelines_for_creating_storage_pools.adoc[Guidelines for creating storage pools]
...
. If you want to use the storage pool in a multi-site Erasure Coding profile, select image:../media/plus_sign_black_on_white.gif[insert icon] to add an entry for each site to the storage pool.
+
...
+
To remove an entry, select image:../media/nms_delete_icon_new.gif[delete icon].
