...
To filter on object sizes smaller than 1 MB, type in a decimal value. For example, set the *Object Size (MB)* advanced filter to *greater than 0.2* to ensure that erasure coding is not used for objects 200 KB or smaller.
