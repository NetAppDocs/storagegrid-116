...
To restore object data, you run the `repair-data` script. This script begins the process of restoring object data and works with ILM scanning to ensure that ILM rules are met. You use different options with the `repair-data` script, based on whether you are restoring replicated data or erasure-coded data.

NOTE: For more information on using the `repair-data` script, enter `repair-data --help` from the command line of the primary Admin Node.
...
IMPORTANT: The `show-replicated-repair-status` option is available for technical preview in StorageGRID 11.6. The value returned by this option is only an estimate.
...
== Find hostname for the Storage Node
...
== If all volumes have failed

If all storage volumes have failed, repair the entire node. Run either or both of the following commands, based on whether you use replicated data, erasure-coded data, or both.

If only some volumes have failed, go to <<If only some volumes have failed>>.
...
== If only some volumes have failed

If only some of the volumes have failed, repair the affected volumes. Run either or both of the following commands, based on whether you use replicated data, erasure-coded data, or both.

If all volumes have failed, go to <<If all volumes have failed>>.
...
== Monitor repairs
Monitor the status of the repair jobs.
