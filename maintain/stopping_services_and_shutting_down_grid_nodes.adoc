...
link:linux_migrating_grid_node_to_new_host[Linux: Migrating a grid node to a new host]
...
[start=4]
. Repeat the previous two steps (<<log_in_to_gn,2>> and <<stop_all_services,3>>) to stop the services on all Storage Nodes, Archive Nodes, and non-primary Admin Nodes.
+
...
. Repeat steps <<log_in_to_gn,2>> and <<stop_all_services,3>> to stop the services on the primary Admin Node.
