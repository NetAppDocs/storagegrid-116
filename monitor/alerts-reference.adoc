...
. From the Grid Manager, go to *CONFIGURATION* > *Audit messages*.
...
. Select *MAINTENANCE > Object existence check > Active job*.
...
.. Ensure there are no service or volume failures on the Storage Nodes. If a service is not running, start or restart the service. See the recovery and maintenance instructions.
...
.. Recover the failed volume. See the recovery and maintenance instructions.
.. Select *Retry*.
...
. Select *MAINTENANCE > Object existence check > Active job*.
. Ensure that all Storage Nodes and volumes being checked are online and available.
. Ensure that sufficient Storage Nodes are online and available to allow the current coordinator node to read object metadata using the selected consistency control. If necessary, start or restart a service. See the recovery and maintenance instructions.
+
