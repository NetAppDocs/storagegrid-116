...
. xref:../admin/configuring-custom-server-certificate-for-grid-manager-tenant-manager.adoc#upload-a-custom-management-interface-certificate[Upload a new management interface certificate.]
...
. xref:../admin/configuring-custom-server-certificate-for-storage-node-or-clb.adoc#upload-a-custom-s3-and-swift-api-certificate[Upload a new S3 and Swift API certificate.]
...
. After resolving any storage issues, run object existence check to determine if objects are missing and to replace them if possible.
. Monitor this alert. The alert will clear after 24 hours, but will be triggered again if the issue has not been fixed.
. If you cannot resolve the alert, contact technical support.

xref:troubleshooting-storagegrid-system.adoc[Running object existence check]
