...
. Select *SUPPORT* > *Metrics*, and select the link to access Prometheus.
. Execute the following:
+
`storagegrid_storage_volume_maximum_optimized_soft_readonly_watermark`
+
The last column is the maximum optimized value of the watermark for all volumes on each node.

. Note the highest optimized value for each Storage Node in your grid.

. Go to the *Nodes* page.

. For each online Storage Node, select *Storage* and go to the Object Stores table.

. Compare the *Available* value for each object store to the highest optimized value for the node.

. If at least one volume has more space available than the highest optimized value, use the optimized watermarks:

.. Go to *Configuration* > *Storage options* > *Configuration*.
.. Change all Watermark Overrides to *0*.

. If no volume on any Storage Node has more space available than the highest optimized value for that node, expand your grid. Add storage volumes or add new Storage Nodes.

. To continue using the current overrides, silence or disable the alert.
...
a|An S3 client is attempting to perform a PUT Object operation that does not meet AWS size limits.

. Use the tenant ID shown in the alert details to identify the tenant account.
. Use the IP address shown in the alert details to identify the specific S3 client.
. Inform all client users that the maximum PUT Object operation size is 5 GiB.
. Consider using multipart uploads for objects larger than 5 GiB.
