...
. If Cassandra is not running, follow the steps for starting or restarting a service in the xref:../maintain/index.adoc[recovery and maintenance instructions].
...
. If Cassandra Reaper is not running, follow the steps for starting or restarting a service in the xref:../maintain/index.adoc[recovery and maintenance instructions].
...
. Select *Edit* to xref:../admin/configuring_administrator_client_certificates.adoc[upload or generate a new certificate].
...
. xref:../admin/configuring_custom_server_certificate_for_grid_manager_tenant_manager.adoc#upload-a-custom-management-interface-certificate[Upload a new management interface certificate.]
...
. xref:../admin/configuring_custom_server_certificate_for_storage_node_or_clb.adoc#upload-a-custom-s3-and-swift-api-certificate[Upload a new S3 and Swift API certificate.]
...
. Review the settings on the Identity Federation page. Confirm that all values are current. See xref:../admin/using_identity_federation.adoc[Using identity federation] in the instructions for administering StorageGRID.
...
. Review the settings on the Identity Federation page. Confirm that all values are current. See xref:../tenant/configuring_federated_identity_source.adoc[Configuring a federated identity source] in the instructions for using a tenant account.
. Click *Test Connection* to validate the settings for the LDAP server.
. If you cannot resolve the issue, contact technical support.
...
xref:troubleshooting_storagegrid_system.adoc[Troubleshooting the Low metadata storage alert]
...
xref:troubleshooting_storagegrid_system.adoc[Troubleshooting the Network Receive Error (NRER) alarm]
...
+
xref:troubleshooting_storagegrid_system.adoc[Troubleshooting lost and missing object data]
...
xref:troubleshooting_storagegrid_system.adoc[Running foreground verification]
