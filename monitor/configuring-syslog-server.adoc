...
* You have a syslog server with the capacity to receive and store the log files. For more information, see xref:../monitor/considerations-for-external-syslog-server[Considerations for external syslog server].
