...
+
CAUTION: When logs are sent to an external syslog server, single logs greater than 8K are truncated at the end of the message. 
...
+
The following are facility passthrough values you can use to select for records that comprise the application logs on the external syslog server: 
