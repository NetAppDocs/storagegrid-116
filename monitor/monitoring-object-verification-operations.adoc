...
.About this task
Two xref:troubleshooting-storagegrid-system.adoc[verification processes] work together to ensure data integrity:
...
Object existence check verifies whether all expected replicated copies of objects and copies of erasure-coded fragments exist on a Storage Node. Object existence check provides a way to verify the integrity of storage devices, especially if a recent hardware issue could have affected data integrity.
