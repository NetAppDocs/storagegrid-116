...
** *Tenant summary*
+
The summary area for the tenant contains values for object count, quota utilization, logical space used, and the quota setting.

 ** *Space breakdown -- Bucket space consumption*
+
The Space breakdown tab includes values for bucket (S3) or container (Swift) space consumption as well as space used and object count for each bucket or container.
+
If a quota was set for this tenant, the amount of quota used and remaining is displayed in text (for example, `85.00 GB of 100 GB used`). If no quota was set, the tenant has an unlimited quota, and the text includes only an amount of space used (for example, `85.00 GB used`). The bar chart shows the percentage of quota in each bucket. If the tenant has exceeded the storage quota by more than 1% and by at least 1 GB, the chart shows the total quota and the excess amount.
+
You can place your cursor over the bar chart to see the storage used by each bucket. You can place your cursor over the free space segment to see the amount of storage quota remaining.
+
