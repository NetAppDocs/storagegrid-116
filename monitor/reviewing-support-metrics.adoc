...
IMPORTANT: The tools available on the Metrics page are intended for use by technical support. Some features and menu items within these tools are intentionally non-functional and are subject to change. See the xref:alerts-reference.adoc#commonly-used-prometheus-metrics.adoc[list of commonly used Prometheus metrics].
