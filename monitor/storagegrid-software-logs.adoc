...
== Metrics collection and other non-log files

[cols="1a,3a" options="header"]
|===
| Metric or file name| Description

m|audit
|Audit messages generated during normal system operation.

m|base-os-logs
|Base OS information, including SG image versions.

m|bundles
|Configuration information (bundles) collected from an ADC node.

m|ec
|VCSs information on the current node and EC group information by profile id.

m|grid
|General grid logs including debug (bycast) and servermanager logs.
...
m|Metrics
|Services log for Grafana, Jaeger, node exporter and Prometheus.

m|mysql
|The mariaDB database configuration and related logs.

m|net
|Logs generated by networking related scripts and Dynip service.
...
|Log of command output executed in SG container that collects system information such as networking and disk usage.
|===
