...
== Contents of log file collection archive

The log file collection contains the logs that are documented on this web page and additional files that contain metrics and debug command output.
...
|Log of command output executed in the StorageGRID container that collects system information, such as networking and disk usage.
|===
