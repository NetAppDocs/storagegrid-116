---
permalink: monitor/task_updating_the_snmp_agent.html
sidebar: sidebar
keywords: 
...
+
NOTE: If you add or remove a user's authoritative engine ID and that user is currently selected for a destination, you must edit or remove the destination, as described in step <<SNMP_TRAP_DESTINATION,#SNMP_TRAP_DESTINATION>>. Otherwise, a validation error occurs when you save the SNMP agent configuration.
...
+
NOTE: If the user you removed is currently selected for a trap destination, you must edit or remove the destination, as described in step <<SNMP_TRAP_DESTINATION,#SNMP_TRAP_DESTINATION>>. Otherwise, a validation error occurs when you save the SNMP agent configuration.
+
