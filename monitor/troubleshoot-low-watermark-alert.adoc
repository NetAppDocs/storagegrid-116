...
* A storage volume watermark was previously set to a custom value. StorageGRID will not override custom watermark settings with optimized values. However, StorageGRID might trigger the *Low read-only watermark override* alert if a custom value for the Storage Volume Soft Read-Only Watermark is too small.
...
Follow these steps if one or more *Low read-only watermark override* alerts have been triggered.
...
.. Compare the *Available* value for each object store (volume) to the <<maximum_optimized_value,maximum optimized watermark>> for that Storage Node.
