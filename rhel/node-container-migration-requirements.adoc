...
Before performing migration or hypervisor maintenance, shut down the nodes gracefully. See the xref:shutting-down-grid-node.adoc[shut down grid node] instructions.
...
Cold migration is supported. In cold migration, you shut down the StorageGRID nodes before migrating them between hosts. See the xref:shutting-down-grid-node.adoc[shut down grid node] instructions.
