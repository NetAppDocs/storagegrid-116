...
a|
This operation determines if default retention mode is enabled for a bucket and see the default retention period being applied to the bucket.
...
a|
This operation allows you to modify the default retention period for a bucket where the default retention mode was previously enabled. You can also remove previously configured default retention settings.
...
If the S3 Object Lock default retention mode is enabled for a bucket, you can specify a default retention period that is applied to each object added to the bucket.
...
The default retention mode and default retention period are used to calculate the retain-until-date when new object versions ingest to the bucket that do not specify their own `x-amz-object-lock-mode` and `x-amz-object-lock-retain-until-date`.

You have the s3:GetBucketObjectLockConfiguration permission, or be account root, to complete this operation.
...
The response XML will return the following values:
...
a|
Container tag for rules used to specify the default object lock settings for the bucket.

Contains one or more status or Rule elements.

a|
ObjectLockEnabled
a|
Object lock for a bucket can be 'Enabled' or 'Disabled'. 

Included in the ObjectLockConfiguration element.

a|
Rule
a|
Container tag for rules that identify the default retention settings for a bucket.
...
DefaultRetention
a|
Container tag for the default object lock settings for a bucket.
...
Mode
a| 
Object lock only supports Compliance mode for object retention.

Included in the DefaultRetention element.

a|
Days
a|
Default retention period for the bucket in days. 
...
Years
a|
Default retention period for the bucket in years. 
...
The PUT Object Lock Configuration request allows you to modify the default retention period for a bucket where the default retention mode was previously enabled. You can also remove previously configured default retention settings.

The default retention mode and default retention period are used to calculate the retain-until-date when new object versions ingest to the bucket that do not specify their own `x-amz-object-lock-mode` and `x-amz-object-lock-retain-until-date`.

If the default retention period is modified after ingest of an object version, the retain-until-date of the object version remains the same and is not recalculated using the new default retention period.

You have the s3:PutBucketObjectLockConfiguration permission, or be account root, to complete this operation.

==== Request

The PUT request must contain the following parameters and values.

NOTE: The `content-MD5` request header must be specified in PUT request.
...
ObjectLockConfiguration
a|
Container tag for rules used to specify the default object lock settings for the bucket. This container must be included in the PUT request.
...
ObjectLockEnabled
a|
Object lock status for a bucket can be 'Enabled' or 'Disabled'. This parameter must be specified in the PUT request.

*Note*: Object lock can only be 'Enabled' or 'Disabled' (initial capital) during bucket creation. The value specified in the PUT request must match the value set during bucket creation. ('Enabled' is required for S3 Object Lock.) 
...
If the Rule parameter is omitted from the PUT request, the default object lock values for the bucket are removed.
...
DefaultRetention
a|
Container tag for the default object lock settings for a bucket. This container must be included in the PUT request.
...
If the default retention mode is Governance, it can be set to Compliance by setting the Mode parameter to COMPLIANCE (all capitals)+
`<Mode>COMPLIANCE</Mode>` 
...
* A default retention period must be specified in days or years, but not in both days and years.
* Days must be an integer value between 1 and 36500, inclusive (100 years).
* Days are 24 hours.
...
* A default retention period must be specified in days or years, but not in both days and years.
* Years must be an integer value between 1 and 100, inclusive.
* Years are calendar years (not always 365 days).
