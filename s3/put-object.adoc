...
+ 
NOTE: If S3 object lock is enabled at the bucket level and a default retention mode or retain-until-date is specified, these default values are assigned to all objects added to the bucket that do not specify their own `x-amz-object-lock-mode` and `x-amz-object-lock-retain-until-date`. +
