...
NOTE: The `content-MD5` request header must be specified in PUR request.
...
If the default retention mode is Governance, it can be it to Compliance by setting the Mode parameter to COMPLIANCE (all capitals)+
`<Mode>COMPLIANCE</Mode>` 
...
* Days must be an integer value between 1 and 36500 (100 years).
* Days are 24 hours.
...
* A default retention period must be specified in days or years, but not in both days and years.
* Years must be an integer value between 1 and 100.
