...
You must have the `Passwords.txt` file to use the Change IP tool.

.Steps

Access the Change IP tool and update the MTU settings as described in xref:../maintain/changing-nodes-network-configuration[Change node network configuration].
