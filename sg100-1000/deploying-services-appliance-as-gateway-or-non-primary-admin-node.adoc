...
+
IMPORTANT: If you have remapped any ports, you cannot use the same ports to configure load balancer endpoints. You can create endpoints using remapped ports, but those endpoints will be remapped to the original CLB ports and service, not the Load Balancer service. Follow the steps in xref:removing-port-remaps.adoc[Remove port remaps].
+
...
xref:automating-appliance-installation-and-configuration.adoc[Automate appliance installation and configuration (SG100 and SG1000)]
...
xref:deploying-services-appliance-as-primary-admin-node.adoc[Deploy services appliance as primary Admin Node]
