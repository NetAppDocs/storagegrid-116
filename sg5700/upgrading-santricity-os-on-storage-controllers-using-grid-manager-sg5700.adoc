...
During the upgrade process, you can skip applying the SANtricity OS upgrade to remaining nodes and finish the upgrade process without upgrading all nodes. Skipped nodes have to be upgraded later. The SANtricity OS upgrade process will not be complete until you approve the SANtricity OS upgrade on all the listed Storage Nodes. If you want to skip upgrading nodes, go to <<Skip Nodes and Finish,Skip Nodes and Finish>> step.
