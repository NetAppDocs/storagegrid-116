...
*** Fix the issues with the node first and then try the upgrade again. 
...
NOTE: During the upgrade process, you can skip applying the SANtricity OS upgrade to remaining nodes and finish the upgrade process without upgrading all nodes. Skipped nodes have to be upgraded later. The SANtricity OS upgrade process will not be complete until you approve the SANtricity OS upgrade on all the listed Storage Nodes. If you want to skip upgrading nodes, go to <<Skip Nodes and Finish,Skip Nodes and Finish>> step.
...
A warning box appears asking if you are sure you want to finish the upgrade process without upgrading all nodes. 

+

image::../media/santricity-skip-nodes.png[screenshot showing the SANtricity OS upgrade skip warning]

[start=2]
.. Select *OK* to return to the *SANtricity OS* page.
+

Nodes already approved and with stage of Complete remain upgraded. 
+

Repeat the upgrade procedure to upgrade the remaining nodes at a later time. 
