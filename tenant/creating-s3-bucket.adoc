...
+
NOTE: The S3 Object Lock permissions are tied to users, not buckets. The same keys can be used to access all buckets or any combination of buckets as determined by the tenant user's xref:bucket-and-group-access-policies.adoc[bucket or group access policy].
...
+
If you enable S3 Object Lock for a bucket, bucket versioning is enabled automatically, and you can specify a default retention mode and default retention period that is applied to each object added to the bucket.
+
