...
+
If you enable S3 Object Lock for a bucket, bucket versioning is enabled automatically. You can also xref:../s3/use-s3-object-lock-default-bucket-retention.adoc[specify a default retention mode and default retention period for the bucket] that are applied to each object ingested to the bucket that does not specify its own retention settings.
