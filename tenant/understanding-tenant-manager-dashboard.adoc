...
* Tenant details, including the tenant account name and ID and whether the tenant can use xref:what-platform-services-are.adoc[platform services], xref:../admin/using-identity-federation.adoc[its own identity source], and xref:../admin/manage-s3-select-for-tenant-accounts.adoc[S3 Select] (only the permissions that are enabled are listed).
