...
You can also specify a default retention mode and default retention period at the bucket level that is applied to each object added to the bucket that does not specify its own retention settings.
+
