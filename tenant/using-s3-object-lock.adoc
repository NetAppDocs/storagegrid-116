...
You can also xref:../s3/use-s3-object-lock-default-bucket-retention.adoc[specify a default retention mode and default retention period for the bucket]. These are applied to each object added to the bucket that does not specify its own retention settings.
