...
Three boolean values have been added to the `grid/logs/collect` endpoint:

* `applicationLogs`: Logs that are frequently needed for troubleshooting. The default is `true`.
