---
permalink: upgrade/generic_whats_new.html
sidebar: sidebar
keywords: 
...
+
NOTE: If your Storage Nodes have enough RAM and sufficient space on volume 0, you can manually increase the Metadata Reserved Space setting up to 8 TB after you upgrade. Reserving additional metadata space after the StorageGRID 11.5 upgrade will simplify future hardware and software upgrades.
+
http://docs.netapp.com/sgws-115/topic/com.netapp.doc.sg-upgrade/home.html[Upgrading StorageGRID] > "`Increasing the Metadata Reserved Space setting`"
+
...
* You can now use the S3 REST API to specify <<OBJECT_LOCK,S3 Object Lock>> settings:
...
* *New audit codes for S3 Object Lock*: Four new audit codes were added to the SPUT audit message to include <<OBJECT_LOCK,S3 Object Lock>> request headers:
