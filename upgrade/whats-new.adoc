...
The user interface for creating an HA group has been enhanced.
...
* You can now specify a failover order for each interface in the HA group. You can choose the active interface and rank the backup interfaces in priority order.
...
The user interface for creating a load balancer endpoint has been enhanced. 
...
An experimental S3 Console has been added to allow tenants to view the objects in an S3 bucket. This experimental console has not been fully tested.

In addition to viewing objects, an S3 tenant can also add and delete objects and object versions; rename objects; move and copy objects between buckets; and manage object tags. The experimental S3 Console is not intended for bulk management of objects or for use in a production environment. Instead, tenants should use S3 Console when performing these functions for a small number of objects, such as when uploading objects to simulate a new ILM policy, troubleshooting ingest issues, or using proof-of-concept or non-production grids.
...
You can now configure an external syslog server if you want to save and manage audit messages and a subset of StorageGRID logs remotely. After an external syslog server is configured, you can save audit messages and certain log files locally, remotely, or both. By being able to configure the destinations of your audit information, you can reduce network traffic on your Admin Nodes. (*CONFIGURATION* > *Monitoring* > *Audit and syslog server*).  
