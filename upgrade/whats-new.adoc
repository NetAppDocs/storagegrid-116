...
* Prometheus and Grafana were updated to newer versions with modified user interface and charts. As part of this change, some metrics were renamed:
** The `node_network_up` metrics are now named `node_network_info`.
** For all `node_network_` metrics (from the netclass collector), the `interface` label name was changed to `device`.
...
+
** xref:../rhel/creating-node-configuration-files.adoc[Create node configuration files for RedHat Enterprise Linux or CentOS deployments]
