...
* You have reviewed the section called xref:planning-and-preparation.adoc[plan and prepare for installation], and you understand the requirements for software, CPU and RAM, and storage and performance.
...
+
You might need to remap a port if your enterprise networking policies restrict access to one or more ports that are used by StorageGRID. See the xref:../network/index.adoc[metworking guidelines] for the ports used by StorageGRID.
+
